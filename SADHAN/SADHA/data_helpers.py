...
class Dataset(object):
    def __init__(self, doc,label, claim, speaker, source):
        self.t_spkr = speaker
        self.t_src = source
        self.t_label = label
        self.t_docs = doc
...
#                self.t_docs.append(line[3].lower())
    ...
    def get_spkr_dom_dict(self):
        spkrdict, domdict = dict(), dict()
        spkridx, domidx = 0, 0
        ...
        if 'Donald Trump' in spkrdict:
            ...
            print("not found")
        return spkrdict, domdict

    def genBatch(self, spkrdict, domdict, wordsdict, batch_size, max_sen_len, max_doc_len, n_class):
...
#            pass
        for i in range(self.epoch):
            ...
            self.sen_len.append(b_sen_len)
            self.doc_len.append(b_doc_len)
    def batch_iter(self, spkrdict, domdict, wordsdict, n_class, batch_size, num_epochs, max_sen_len, max_doc_len, shuffle=True):
        ...
        for epoch in range(num_epochs):
            ...
            for batch_num in range(num_batches_per_epoch):
                ...
                batch_data = zip(spkr, dom, docs, label, sen_len, doc_len)
                yield batch_data
