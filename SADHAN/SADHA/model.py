...
class SADHA(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
            embedding_dim, hidden_size, spkr_num, dom_num):
        ...
        with tf.name_scope('embedding'):
            ...
            self.speaker_embedding = tf.Variable(tf.random_uniform([self.spkr_num, 2*self.hidden_size], -0.01, 0.01), dtype=tf.float32)
            self.domain_embedding = tf.Variable(tf.random_uniform([self.dom_num, 2*self.hidden_size], -0.01, 0.01), dtype=tf.float32)
            self.user = tf.nn.embedding_lookup(self.speaker_embedding, self.spkrid)
            self.product = tf.nn.embedding_lookup(self.domain_embedding, self.domid)
