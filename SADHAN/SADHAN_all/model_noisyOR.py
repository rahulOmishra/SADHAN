...
class SADHA(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        self.alpha_spkr_word = []
        self.alpha_spkr_sent = []
        self.alpha_dom_word = []
        self.alpha_dom_sent = []
        self.alpha_tpc_word = []
        self.alpha_tpc_sent = []
...
#            domain = tf.reshape(self.domain,
#                               shape)
#            speaker = tf.reshape(self.speaker,
#                               shape)
#            outputs = tf.concat([outputs,domain,speaker],1)
...
#                               shape)
            self.alpha_spkr_word.append(alpha)
        ...
        with tf.name_scope('u_sentence_attention'):
            ...
            self.alpha_spkr_sent.append(alpha)
    ...
    def Domain_attention(self):
        ...
        with tf.name_scope('p_word_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['p_v_1']),
                               [batch_size, 1, self.max_sen_len])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            self.alpha_dom_word.append(alpha)
            self.alpha_dom = self.weights['p_v_1']
            outputs = tf.matmul(alpha, outputs)
        ...
        with tf.name_scope('p_sentence_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['p_v_2']),
                               [batch_size, 1, self.max_doc_len])
            alpha = self.softmax(alpha, self.doc_len, self.max_doc_len)
            self.alpha_dom_sent.append(alpha)
    ...
    def topic_attention(self):
        ...
        with tf.name_scope('t_word_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['t_v_1']),
                               [batch_size, 1, self.max_sen_len])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            self.alpha_tpc_word.append(alpha)
            self.alpha_tpc = self.weights['t_v_1']
        ...
        with tf.name_scope('t_sentence_attention'):
            ...
            alpha = tf.reshape(tf.matmul(u, self.weights['t_v_2']),
                               [batch_size, 1, self.max_doc_len])
            alpha = self.softmax(alpha, self.doc_len, self.max_doc_len)
            self.alpha_tpc_sent.append(alpha)
            outputs = tf.matmul(alpha, outputs)
...
#         NOISY-or gate
            ...
            self.loss = 1-((1-tf.reduce_mean(losses))*(1-self.u_loss)*(1-self.p_loss)*(1-self.t_loss))

#            self.loss =  tf.reduce_mean(losses) 
#        with tf.variable_scope("u_word_attention", reuse=True):
#            w = tf.get_variable("wu_1")
#            print(w)


        with tf.name_scope("metrics"):
            correct_predictions = tf.equal(self.predictions, tf.argmax(self.input_y, 1))
#            precision, recall, f1 = score(tf.argmax(self.input_y, 1), self.predictions)
#
#            print('precision: {}'.format(precision))
#            print('recall: {}'.format(recall))
#            print('fscore: {}'.format(f1))
#   
            self.embed_spkr = self.speaker_embedding
            self.embed_dom = self.domain_embedding
            self.embed_tpc = self.topic_embedding
            self.alpha_spkr_word = tf.add(self.alpha_spkr_word, 0,name= "alpha_spkr_word")
            self.alpha_spkr_sent = tf.add(self.alpha_spkr_sent, 0,name= "alpha_spkr_sent")
            self.alpha_dom_word = tf.add(self.alpha_dom_word, 0,name= "alpha_dom_word")
            self.alpha_dom_sent = tf.add(self.alpha_dom_sent, 0,name= "alpha_dom_sent")
            self.alpha_tpc_word = tf.add(self.alpha_tpc_word, 0,name= "alpha_tpc_word")
            self.alpha_tpc_sent = tf.add(self.alpha_tpc_sent, 0,name= "alpha_tpc_sent")
            ...
            self.C_matrix = tf.confusion_matrix(labels = tf.argmax(self.input_y, 1), predictions = self.predictions)
            self.auc = tf.metrics.auc(tf.argmax(self.input_y, 1), self.predictions)
